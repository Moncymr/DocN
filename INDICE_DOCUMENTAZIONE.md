...
- Tre modalità di ricerca (Hybrid, Vector, Text)

### Funzionalità di Ricerca Disponibili ✅
1. **Ricerca Ibrida** - Combina vector + text con RRF
2. **Ricerca Vettoriale** - Similarità semantica avanzata
...
## 🔎 Guida alle Funzionalità di Ricerca e RAG

### Tipi di Ricerca Disponibili

DocN offre tre modalità di ricerca avanzate per trovare documenti nel sistema:

#### 1. **Ricerca Ibrida (Hybrid Search)** - ⭐ CONSIGLIATA
...
**Quando usarla:**
- ✅ Ricerche generiche che richiedono sia precisione concettuale che matching esatto
- ✅ Query in linguaggio naturale ("Trova i contratti firmati quest'anno")
- ✅ Ricerca di concetti simili ma con parole diverse
...
Ricerca basata su similarità semantica usando embeddings vettoriali. Trova documenti concettualmente simili anche se usano parole diverse.
...
**Quando usarla:**
- ✅ Ricerca concettuale ("Trova documenti che parlano di gestione progetto")
- ✅ Query con sinonimi o termini diversi ma stesso significato
- ✅ Quando la semantica è più importante della corrispondenza esatta

**Esempio pratico:**
- Query: "processo rilascio software"
- Documenti trovati:
  - `Attività GitLab per chiusura versione e apertura fix.docx` (alto match semantico)
...
**Quando usarla:**
- ✅ Ricerca di termini specifici o codici esatti
- ✅ Nomi di file conosciuti
- ✅ Quando non hai embeddings generati

**Esempio pratico:**
- Query: "GitLab"
- Documenti trovati:
  - `Attività GitLab per chiusura versione e apertura fix.docx` (match esatto nel nome)

---

### 📊 Confronto Tipi di Ricerca

| Caratteristica | Hybrid Search | Vector Search | Text Search |
|---------------|---------------|---------------|-------------|
| **Precisione** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| **Velocità** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **Ricerca Semantica** | ✅ Sì | ✅ Sì | ❌ No |
| **Match Esatto** | ✅ Sì | ⚠️ Parziale | ✅ Sì |
| **Richiede Embeddings** | ✅ Sì | ✅ Sì | ❌ No |
...
**Architettura RAG:**
```
User Query → Embedding Generation → Vector Search → Document Retrieval
    ↓
Context Building → AI Agent Processing → Response Generation
    ↓
...
2. Vai su "Documents"
3. Carica documenti
4. ✅ Seleziona "Generate Embeddings" durante l'upload
...
```json
{
  ...
  "sourceDocuments": [
    {
      "documentId": 42,
      "fileName": "Attività GitLab per chiusura versione e apertura fix.docx",
      "category": "Technical",
      "similarityScore": 0.89,
      "relevantChunk": "Le attività principali per la chiusura versione includono..."
    }
  ],
  "conversationId": 15,
  "responseTimeMs": 1250
...
### 🎯 Esempi Pratici di Query RAG sui Documenti di Esempio
...
**Documento:** `Attività GitLab per chiusura versione e apertura fix.docx`
...
**Risposta Attesa:**
Il sistema analizzerà tutti i documenti rilevanti e fornirà una sintesi comparativa citando le fonti.

---

### 🏗️ Microsoft Agent Framework - Implementazione Chart RAG

**Domanda:** La chart RAG è implementabile tramite Microsoft Agent Framework?

**Risposta:** ✅ **Sì, è già implementata!**
...
- **Responsabilità:**
  ...
  - Determinare documenti più rilevanti
...
public class SearchOptions
{
    ...
    public double MinSimilarity { get; set; } = 0.7;  // Soglia similarità (0-1)
...
- **MinSimilarity = 0.5**: Più permissivo, più risultati
...
- **"Come funzionano le ricerche e il sistema RAG in DocN?"** ⭐ NUOVO
- **"Come implementare e configurare gli agenti Microsoft per RAG?"** ⭐ NUOVO

La risposta è articolata in documenti che coprono:
...
- ✅ **Guida completa Ricerca e RAG con esempi pratici** ⭐ NUOVO
- ✅ **Implementazione Microsoft Agent Framework per Chart RAG** ⭐ NUOVO

**Tutto il necessario per trasformare DocN in un sistema RAG enterprise production-ready e per utilizzare appieno le sue funzionalità di ricerca avanzata e intelligenza artificiale.**
