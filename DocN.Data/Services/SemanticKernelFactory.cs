    ...
    public async Task<Kernel> CreateKernelAsync()
    {
            ...
            !string.IsNullOrEmpty(config.AzureOpenAIEndpoint) && !string.IsNullOrEmpty(config.AzureOpenAIKey))
        {
            try
            {
                ...
                {
                    var chatDeployment = config.ChatDeploymentName ?? config.AzureOpenAIChatModel ?? "gpt-4";
                ...
                {
                    var embeddingDeployment = config.EmbeddingDeploymentName ?? config.AzureOpenAIEmbeddingModel ?? "text-embedding-ada-002";
            ...
            !string.IsNullOrEmpty(config.OpenAIApiKey))
        {
            try
            {
                ...
                {
                    var chatModel = config.OpenAIChatModel ?? "gpt-4";
                ...
                {
                    var embeddingModel = config.OpenAIEmbeddingModel ?? "text-embedding-ada-002";
